...
// --- Ensure $user_id is set from middleware ---
if (!isset($user_id)) {
...
// --- Fetch user's vertical (selected_template_id) ---
...
// --- Insert product ---
$stmt = $conn->prepare("
    INSERT INTO products (user_id, business_type_id, name, price, unit, created_at) 
    VALUES (?, ?, ?, ?, ?, NOW())
");
$stmt->bind_param("iisds", $user_id, $business_type_id, $name, $price, $unit);

if ($stmt->execute()) {
    echo json_encode(["success" => true, "msg" => "Product added successfully"]);
} else {
